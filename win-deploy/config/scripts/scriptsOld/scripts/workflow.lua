...
function Workflow.IssueCardWorkflow(rawCitizenText)
    ...
    local blockKey = Machine.Read().blockKey

    if blockKey ~= nil and heldCardUid == nil then
        fail(Machine.Describe(blockKey), blockKey)
        return
    end
    ...
    while true do
        ...
        if heldCardUid == nil then
            ...
            local attemptBlock = Machine.BlockKey()

            if attemptBlock ~= nil then
                fail(Machine.Describe(attemptBlock), attemptBlock)
                return
            end
        ...
        local cardUid, wasHeld = acquireCard()
        ...
        if cardUid == nil then
            ...
            if not Utils.IsSuccess(regResult) then
                ...
                Plc.MoveCardIn()
                Plc.MoveCardOut()
                local taken = Plc.WaitCardTaken(Config.TAKE_CARD_TIMEOUT)
