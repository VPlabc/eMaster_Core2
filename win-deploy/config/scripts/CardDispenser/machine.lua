...
function Machine.BlockKey(machine)

    if machine == nil or machine == "staff" then
        return Machine.Read().blockKey
    end

    local io = Config.MACHINES[machine]
    if io == nil then
        return Machine.Read().blockKey
    end

    local hopperEmpty = Plc.ReadInput(io.input_hopper_empty)
    Variables.SetFlag("CardNotInHopper_" .. machine, hopperEmpty == true)
...
function Machine.Describe(ledKey, machine)

    if machine == "contractor" and ledKey == "HOPPER_EMPTY" then
        local io = Config.MACHINES.contractor
        return "No card in contractor hopper (input " .. tostring(io.input_hopper_empty) .. ")"
    end
